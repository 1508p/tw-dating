...
!function(window, nameSpace) {
    ...
    function refreshRem(reset) {
        ...
        var resetRem = function(){
            ...
            if(!doc.body || resetNum > 0) return;

            var remdiv = doc.createElement('div');
                remdiv.id = 'remdiv';
                remdiv.style.width = '10rem';
            doc.body.appendChild(remdiv);

            // console.log(remdiv.offsetWidth , width);
            // console.log(remdiv.offsetWidth , width);

            if(remdiv.offsetWidth != width){
                var ratio = remdiv.offsetWidth / width;
                var reset = (rootSize / ratio).toFixed(4);
                remdiv.remove();
                rootSize = reset;
            }else{
                remdiv.remove();
    ...
    if (docEl.setAttribute("data-dpr", dpr), !metaEl) if (metaEl = doc.createElement("meta"), metaEl.setAttribute("name", "viewport"), //j = scale            //j = scale              //j = scale
    metaEl.setAttribute("content", "initial-scale=" + scale + ", maximum-scale=" + scale + ", minimum-scale=" + scale + ", user-scalable=no"), docEl.firstElementChild) docEl.firstElementChild.appendChild(metaEl);
    else {
    ...
    function(a) {
        a.persisted && (clearTimeout(timer), timer = setTimeout(refreshRem, 300))
    },
    !1),

    "complete" === doc.readyState ? doc.body.style.fontSize = 12 * dpr + "px": doc.addEventListener("DOMContentLoaded",
    function() {
    ...
    Flexible.rem2px = function(a) {
        ...
        return "string" == typeof a && a.match(/rem$/) && (pxValue += "px"),
        pxValue;
    },
    Flexible.px2rem = function(a) {
        var remValue = parseFloat(a) / this.rem;
        return "string" == typeof a && a.match(/px$/) && (remValue += "rem"),
        remValue;
    }
